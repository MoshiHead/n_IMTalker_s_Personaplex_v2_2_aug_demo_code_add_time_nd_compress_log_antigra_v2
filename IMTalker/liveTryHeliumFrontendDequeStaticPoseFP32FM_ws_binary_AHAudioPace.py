    ...
    async def conversation(ws: WebSocket):
        ...
        try:
            while True:
                ...
                elif msg_type == "client_audio_latency":
                    ...
                    if reply_engine is not None and turn_id is not None:
                        with contextlib.suppress(Exception):
                            ...
                            if rec is not None:
                                ...
                                real_first_lat = t_send_offset + s_to_c + client_tot
                                client_delays = {
                                    "server_send_to_client_recv": s_to_c,
                                    "client_recv_to_audio_buffer": c_recv_to_buf,
                                    "audio_buffer_to_playback": buf_to_play,
                                }
                                biggest_stage, biggest_val = max(client_delays.items(), key=lambda kv: kv[1])
                                print(
                                    f"\n[CLIENT LATENCY] TURN {turn_id}:\n"
                                    f"  REAL FIRST AUDIO LATENCY: {real_first_lat:.3f}s\n"
                                    f"  BIGGEST CLIENT-SIDE DELAY: {biggest_stage} = {biggest_val:.3f}s\n",
                                    flush=True,
                                )
                    continue
