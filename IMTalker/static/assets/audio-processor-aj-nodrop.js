...
class MoshiProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    if (this.firstOut) {
      this.firstOut = false;
      if (out_idx > 0) {
        // Additive, logging-only: tell the main thread this callback just
        // actually copied real decoded samples to the output for the first
        // time since start()/resetStart() -- the only place true playback
        // start is knowable (buffering/underrun logic above is unchanged).
